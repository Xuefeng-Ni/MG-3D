...
def _make_nConv(in_channel, depth, act, double_chnnel=False, eval_bn=False):
    if double_chnnel:
        layer1 = LUConv(in_channel, 48 * (2 ** (depth+1)),act, eval_bn)
        layer2 = LUConv(48 * (2 ** (depth+1)), 48 * (2 ** (depth+1)),act, eval_bn)
    else:
        layer1 = LUConv(in_channel, 48*(2**depth),act, eval_bn)
        layer2 = LUConv(48*(2**depth), 48*(2**depth)*2,act, eval_bn)
...
class UNet3D_Encoder(nn.Module):
    ...
    def __init__(self, in_channels=1, act='relu', eval_bn=False):
        ...
        '''
        self.down_tr64 = DownTransition(in_channels,0, act, eval_bn)
        self.down_tr128 = DownTransition(96,1,act, eval_bn)
        self.down_tr256 = DownTransition(192,2,act, eval_bn)
        self.down_tr512 = DownTransition(384,3,act, eval_bn)
