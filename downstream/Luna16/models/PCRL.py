...
class PCRLModel3d(nn.Module):
    ...
    def forward(self, x1, x2, aug_tensor1, aug_tensor2, aug_tensor_h):
        ...
        x0_out_o, x1_out_o, x2_out_o, x3_out_o, x4_out_o  = feats_q
        x0_out_ema, x1_out_ema, x2_out_ema, x3_out_ema, x4_out_ema = feats_k
        # from layer_i to layer_L
        xh = alpha * x1 + (1 - alpha) * x2
        x0_out_alpha = alpha * x0_out_o + (1 - alpha) * x0_out_ema
        x1_out_alpha = alpha * x1_out_o + (1 - alpha) * x1_out_ema
        x2_out_alpha = alpha * x2_out_o + (1 - alpha) * x2_out_ema
        x3_out_alpha = alpha * x3_out_o + (1 - alpha) * x3_out_ema
        x4_out_alpha = alpha * x4_out_o + (1 - alpha) * x4_out_ema

        feats_mixed = [x0_out_alpha, x1_out_alpha, x2_out_alpha, x3_out_alpha, x4_out_alpha]
        feat_mixed = self.encoder(x4_out_alpha, mixup=True)

        # Decoder
        Pre_To_x1 = self.decoder(x1, feats_q, aug_tensor1)
        Pre_Tm_x2 = self.decoder(x2, feats_k, aug_tensor2)
        Pre_Th_x = self.decoder(xh, feats_mixed, aug_tensor_h)
        '''
