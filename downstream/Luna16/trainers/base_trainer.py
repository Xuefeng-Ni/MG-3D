...
class BaseTrainer(object):
    ...
    def get_training_phase(self):
        ...
        if self.config.pretrained_model is not None:
            ...
            if self.config.network == 'unet_3d_dense':
            ...
            elif self.config.network == 'swin_3d':
                self.transferred_dict = ['patch_embed', 'layers1', 'layers2', 'layers3', 'layers4']
    ...
    def backward(self):
        ...
        self.loss = self.criterion(self.pred, self.target.clip(min=0,max=1))
    ...
    def load_model_state_dict(self, load_from: str):
        ...
        #         {'module.'+ k: v for k, v in self.resume_checkpoint.items()}, strict=True)
        # else:
        self.model.load_state_dict(self.resume_checkpoint, strict=True)
...
##        self.optimizer.load_state_dict(self.resume_checkpoint['optimizer'])
    ...
    def load_pretrained_weights(self):
        ...
        if ('swin' in self.config.note):   # or ('mrm' in self.config.note)

            extractor_dict = checkpoint

            transferred_state_dict = {}
            for key, value in extractor_dict.items():
                name = key[15:]
                if key[:14] == 'vision_encoder':
                    transferred_state_dict[name] = value

            model_dict = dict(self.model.module.state_dict())
            pretrain_dict = {k: v for k, v in transferred_state_dict.items() if k in model_dict}
            self.model.module.load_state_dict(pretrain_dict, strict=False)
