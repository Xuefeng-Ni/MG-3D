...
class PCRLLunaPretaskSet_ORIG(PCRLBase):
    ...
    def __init__(self, config, base_dir, flag=False):
        ...
        self.all_images = self.get_file_list(split=self.flag)
...
class PCRLLunaPretaskSet(Dataset):
    ...
    def get_file_list(self, split='train'):
        from monai.data import load_decathlon_datalist
        datalist_json = os.path.join('/home/xuefeng/chest_ctrg_r2gen_nitfy_ssl.json')
        if split == 'train':
            ...
            datalist = load_decathlon_datalist(datalist_json,
                                                ...
                                                base_dir=self._base_dir)         
        all_images = []
        subset = os.path.join(self._base_dir)
        for i in range(len(datalist)):
            file_name = datalist[i]['image'].split('/')[-1][:-7]
            for file in glob(os.path.join(subset, file_name + '_' + '*.npy')):
                all_images.append(file)
        '''
        for i in self.folds:
            subset = os.path.join(self._base_dir, 'subset' + str(i))
