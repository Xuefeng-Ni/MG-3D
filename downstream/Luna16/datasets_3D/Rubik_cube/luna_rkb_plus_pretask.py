...
class RKBPLunaPretaskSet(RKBBase):
    ...
    def __init__(self, config, base_dir, flag):
        ...
        # load data from .npy
        if self.flag == 'train':
            self.get_luna_list(split='train')
        else:
            self.get_luna_list(split='val')
    ...
    def get_luna_list(self, split):
        ...
        for i in range(len(datalist)):
            file_name = datalist[i]['image'].split('/')[-1][:-7] + ".npy"
            print('***file_name**:', file_name)
            file_list = glob.glob(os.path.join(self.base_dir, file_name))
            for img_file in tqdm(file_list):
                self.all_images.append(img_file)
