...
class SwinTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.proj_head = projection_head(in_dim=1536, hidden_dim=768, out_dim=768)
...
##        self.head = nn.Linear(1536, num_classes) if num_classes > 0 else nn.Identity()
        self.norm = norm_layer(1536)
        self.head = nn.Sequential(nn.Linear(1536, 768),
                                    nn.LayerNorm(768), 
                                nn.GELU(),
                                nn.Linear(768, num_classes)) # output layer # used for simsiam loss
    ...
    def forward(self, x, normalize=True):   #, hashfile
        ...
        # x4_out = rearrange(x4_out, 'b c d h w -> b (d h w) c')
        # x4_out = self.proj_head(x4_out)
        # x4 = rearrange(x4_out, 'b (d h w) c -> b c d h w', d=1, h=12, w=12)
...
##        out = self.avgpool(out.transpose(1, 2))  # B C 1
        ...
        return out
    '''
    def forward(self, x, hashfile, normalize=True):
        with torch.no_grad():
    ...
    #         out = torch.flatten(out, 1)
            ...
            cache_dir = Path('/data/nixuefeng/ccii_cache_rate4.6k/')       

            outs = []
            for b in range(x.size(0)):
                item_transformed = hashfile[b]
                if cache_dir is not None:
                    data_item_md5 = pickle_hashing(item_transformed).decode("utf-8")
                    hashfile_b = cache_dir / f"{data_item_md5}.pt"
                if hashfile_b is not None and hashfile_b.is_file():  # cache hit
                    outs.append(torch.load(hashfile_b).cuda())
                else:        
                    try:
                        x0 = self.patch_embed(x[b].unsqueeze(0))
                        x0 = self.pos_drop(x0)
                        # x0_out = self.proj_out(x0, normalize)
                        # if self.use_v2:
                        #     x0 = self.layers1c[0](x0.contiguous())
                ...
                ##        out = self.avgpool(out.transpose(1, 2))  # B C 1
                        ...
                        with tempfile.TemporaryDirectory() as tmpdirname:
                            temp_hash_file = Path(tmpdirname) / hashfile_b.name
                            torch.save(
                                obj=out.detach().cpu(),
                                f=temp_hash_file,
                            ...
                            if temp_hash_file.is_file() and not hashfile_b.is_file():
                                shutil.move(str(temp_hash_file), hashfile_b)
