...
from networks.lib.loss.loss_helper import FSAuxOhemCELoss, FSOhemCELoss, FSRMILoss
from networks.lib.loss.loss_helper import FSCELoss, FSAuxCELoss, FSAuxRMILoss, FSCELOVASZLoss, MSFSAuxRMILoss, FSAuxCELossDSN
from networks.lib.loss.loss_helper import SegFixLoss
from networks.lib.loss.rmi_loss import RMILoss
from networks.lib.loss.loss_contrast import ContrastAuxCELoss, ContrastCELoss
from networks.lib.loss.loss_contrast_mem import ContrastCELoss as MemContrastCELoss

from networks.lib.utils.tools.logger import Logger as Log
from networks.lib.utils.distributed import is_distributed
...
class LossManager(object):
    ...
    def _parallel(self, loss):
        ...
        if self.configer.get('network', 'loss_balance') and len(self.configer.get('gpu')) > 1:
            ...
            from networks.lib.extensions.parallel.data_parallel import DataParallelCriterion
