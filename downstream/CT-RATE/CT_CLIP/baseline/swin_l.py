...
from baseline.swin_vit import SwinTransformer as SwinViT
...
class SwinTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.num_features = 27    #32  #30  #36  #49  #36  #27
...
##        self.head = nn.Linear(self.num_features, num_classes) if num_classes > 0 else nn.Identity()
    ...
    def forward(self, x, normalize=True):
        ...
        out = self.norm(out).permute(0,2,1)  # B L C (4, 27, 768)
        out_avg = self.avgpool(out.transpose(1, 2))  # B C 1 (4, 768)
        out_avg = torch.flatten(out_avg, 1)

        # print(out.shape, out_avg.shape)
        return out, out_avg
...
class Swin(nn.Module):
    ...
    def forward_encs(self, encs):
        ...
        outs = []
        for enc in encs:
            out = F.adaptive_avg_pool3d(enc, (4, 4, 3))
            outs.append(out)
        outs = torch.cat(outs[:-1], dim=1)
        outs = outs.permute(0,3,4,2,1).contiguous()
        return [outs, encs[-1].permute(0,3,4,2,1).contiguous()]

    def forward(self, x_in, hashfile, device, return_encoded_tokens=False):
        b = x_in.size()[0]
        hidden_states_out = self.swinViT(x_in, hashfile, device, return_encoded_tokens)
