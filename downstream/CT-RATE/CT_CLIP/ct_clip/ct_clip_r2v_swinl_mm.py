...
class CTCLIP_L(nn.Module):
    ...
    def forward(
        ...
        enc_text, enc_image = text, [image_1, image_2]
        
        for i in range(len(enc_image)):
            uni_modal_image_feats = rearrange(enc_image[i], 'b d h w c -> b (d h w) c')
            uni_modal_image_feats = self.multi_modal_vision_proj(uni_modal_image_feats)

            x = torch.mean(enc_text, dim=1).unsqueeze(0)
            ...
            enc_image[i] = rearrange(multi_modal_image_feats, 'b (d h w) c -> b d h w c', d=4, h=3, w=4)

        enc_image = torch.cat(enc_image, dim=-1)
