...
class CTCLIP_B(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.local_img_cross_multihead_attention = nn.ModuleList(
            [MIMCrossMultiHeadAttention(768, 128) for _ in range(2)])
        self.local_text_cross_multihead_attention = nn.ModuleList(
            [MLMCrossMultiHeadAttention(768, 128) for _ in range(2)])

        self.multi_modal_vision_proj = nn.Linear(768, 768)
        self.modality_type_embeddings = nn.Embedding(2, 768)
    ...
    def forward(
        ...
        b, device = text.shape[0], device

        # derive text mask

        enc_text, enc_image = text, image
        
        uni_modal_image_feats = rearrange(enc_image, 'b d h w c -> b (d h w) c')
        ...
        uni_modal_text_feats, uni_modal_image_feats = (
            enc_text + self.modality_type_embeddings(torch.zeros([b, 512]).to(torch.int64).to(device)),
            uni_modal_image_feats + self.modality_type_embeddings(torch.full_like(image_masks, 1).to(device)),
        )
        x = torch.mean(uni_modal_text_feats, dim=1).unsqueeze(0)
        y = uni_modal_image_feats
        ...
        for layer_idx, layer in enumerate(self.local_img_cross_multihead_attention):
            ...
            for idx in range(y.size(0)):
                local_text_embed = x # get sentence-level representation 
        ...
        enc_image = rearrange(multi_modal_image_feats, 'b (d h w) c -> b d h w c', d=4, h=3, w=4)
...
##        text_embeds = text_embeds[:,0,:]
        text_embeds = multi_modal_text_feats.squeeze(0)
