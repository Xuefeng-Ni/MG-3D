...
##from ct_clip.ct_clip_feature import CTCLIP
                ...
                for model, filename in models_to_evaluate:
                    ...
                    for i in tqdm.tqdm(range(len(self.ds))):
                        ...
                        text_tokens=self.tokenizer(
                                        list(text), return_tensors="pt", padding="max_length", truncation=True, max_length=250).to(device)
                        ...
                        for idx, pathology in enumerate(pathologies):
                            ...
                            text_tokens=self.tokenizer(
                                            text, return_tensors="pt", padding="max_length", truncation=True, max_length=250).to(device)

                            output = model(text_tokens, valid_data.cuda(), name_all, device=device)
