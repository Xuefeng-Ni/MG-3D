        ...
        if True:
            with torch.no_grad():
                ...
                for model, filename in models_to_evaluate:
                    ...
                    text_latent_list = []
                    image_latent_list, image_latent_list_1, image_latent_list_2 = [], [], []
                    accession_names=[]
                    ...
                    for i in tqdm.tqdm(range(len(self.ds))):
                        ...
                        text_latents, image_latents = model(text_tokens, valid_data.cuda(), name_all, device=device, return_latents=True)                        
                        text_latent_list.append(text_latents.detach().cpu().numpy())
                        image_latent_list_1.append(image_latents[0].detach().cpu().numpy())
                        image_latent_list_2.append(image_latents[1].detach().cpu().numpy())

                        plotdir = self.result_folder_txt
                        Path(plotdir).mkdir(parents=True, exist_ok=True)


                    np.savez(f"{plotdir}text_latents_order.npz", data=np.array(text_latent_list))
                    np.savez(f"{plotdir}image_latents_order_1.npz", data=np.array(image_latent_list_1))                  
                    np.savez(f"{plotdir}image_latents_order_2.npz", data=np.array(image_latent_list_2))                  
