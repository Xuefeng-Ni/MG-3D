...
class CTClipInference(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.img_latents = np.load(f"{self.results_folder}/image_latents_order.npz")["data"][:,0,:]
        ...
        if True:
            with torch.no_grad():
                ...
                for model, filename in models_to_evaluate:
                    ...
                    text_latent_list_all = []
                    image_latent_list_all = []
                    accession_names=[]
                    ...
                    for i in tqdm.tqdm(range(len(self.ds))):
                        valid_data, text, onehotlabels, acc_name, name_all = next(self.dl_iter)
                        ...
                        self.img_latents = torch.tensor(self.img_latents).to(device)
                        text_latent_list = []
                        image_latent_list = []
                        
                        for j in range(self.img_latents.size(0)):
                            
                            image_latents_ori = self.img_latents[j, :].unsqueeze(0)

                            text_tokens=self.tokenizer(
                                            list(text), return_tensors="pt", padding="max_length", truncation=True, max_length=512).to(device)
                            text_latents, image_latents = model(text_tokens, valid_data.cuda(), image_latents_ori, name_all, device=device, return_latents=True)                        

                            text_latent_list.append(text_latents.detach().cpu().numpy())
                            image_latent_list.append(image_latents.detach().cpu().numpy())
                        
                        text_latent_list_all.append(text_latent_list)
                        image_latent_list_all.append(image_latent_list)
                        
                        plotdir = self.result_folder_txt
                        Path(plotdir).mkdir(parents=True, exist_ok=True)


                    np.savez(f"{plotdir}text_latents_order_mm.npz", data=np.array(text_latent_list_all))
                    np.savez(f"{plotdir}image_latents_order_mm.npz", data=np.array(image_latent_list_all))                  
