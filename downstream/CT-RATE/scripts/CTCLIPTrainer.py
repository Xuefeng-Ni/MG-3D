...
class CTClipTrainer(nn.Module):
    ...
    def train_step(self):
        ...
        # update CTClip model
        video, text, acc_name = next(self.dl_iter)
        print(video.shape)
        device=self.device
        ...
        with self.accelerator.autocast():
            loss = self.CTClip(text_tokens, video, acc_name, return_loss=True, device=device)
                ...
                for model, filename in models_to_evaluate:
                    ...
                    for i in range(100):
                        ...
                        for pathology in pathologies:
                            ...
                            output = model(text_tokens, valid_data, name_acc, device=device)
