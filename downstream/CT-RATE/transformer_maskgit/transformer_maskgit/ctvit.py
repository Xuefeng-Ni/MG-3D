...
class CTViT(nn.Module):
    ...
    def forward(
        ...
        *_, h, w, _ = shape
        ...
        '''
        cache_dir = Path('/home/xuefeng/cache/ct_vocab_cache_ct_clip_official/')       
        token_list = []
        for b in range(video.size(0)):
            item_transformed = filename[b]
            ...
            if hashfile_b is not None and hashfile_b.is_file():  # cache hit
                hashfile_b = torch.load(hashfile_b).to(device)
                shape = hashfile_b.shape
                *_, h, w, _ = shape
                token_list.append(hashfile_b)
            else:        
                try:
                    token = self.to_patch_emb(video[b].unsqueeze(0))
                    # save height and width in
