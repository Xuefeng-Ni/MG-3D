...
def pre_process(img, min=-175, max=250):
    output = img.copy()
    output[img < min] = min
    output[img > max] = max
...
def vis():
    path = '/jhcnas1/nixuefeng/MSD/exps/fold0_swin/pred_fold0/'
    img_path = path
    label_path = path
    pred_path = path
    ...
    for i in ls:
        ...
        lab = read(os.path.join(label_path, i[:-7]+'_label.nii.gz'), True)[0].astype(np.uint8)
        pred = read(os.path.join(pred_path, 'pred_'+i[:-7]+'_label.nii.gz'), True)[0].astype(np.uint8)
