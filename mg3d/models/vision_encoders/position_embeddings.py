...
def get_3d_sincos_pos_embed_2(embed_dim, grid_size, cls_token=False):
    """
    grid_size: int of the grid height and width
    return:
    pos_embed: [grid_size*grid_size, embed_dim] or [1+grid_size*grid_size, embed_dim] (w/ or w/o cls_token)
    """
    grid_d = np.arange(8, dtype=np.float32)
    grid_h = np.arange(6, dtype=np.float32)
...
##    grid = grid.reshape([2, 1, grid_size, grid_size])
##    grid = grid.reshape([3, 1, grid_size, grid_size, grid_size])
    grid = grid.reshape([3, 1, 8, 8, 6])
    pos_embed = get_3d_sincos_pos_embed_from_grid_2(embed_dim, grid)
    if cls_token:
        pos_embed = np.concatenate([np.zeros([1, embed_dim]), pos_embed], axis=0)
    return pos_embed
...
def get_3d_sincos_pos_embed_from_grid_2(embed_dim, grid):
    assert embed_dim % 3 == 0

    # use half of dimensions to encode grid_h
    emb_d = get_1d_sincos_pos_embed_from_grid(embed_dim // 3, grid[0])  # (H*W, D/2)
    emb_w = get_1d_sincos_pos_embed_from_grid(embed_dim // 3, grid[1])  # (H*W, D/2)
    emb_h = get_1d_sincos_pos_embed_from_grid(embed_dim // 3, grid[2])  # (H*W, D/2)
