...
class LocalCrossMultiHeadAttention(nn.Module):
    def __init__(self, embed_dim, drop_rate=0):
        super(LocalCrossMultiHeadAttention, self).__init__()
        self.embed_dim = embed_dim
        self.query1 = nn.Linear(embed_dim, embed_dim)
        self.key1 = nn.Linear(embed_dim, embed_dim)
        self.value1 = nn.Linear(embed_dim, embed_dim)
...
##        context_layer1 =  attention_probs1 @ value_layer1 # [T, I] @ [I, D] = [T, D]
        ...
        for i in range(attention_probs2.size(0)):
            context_layer2.append(self.avgpool(context_layer2_whole[i, :, :].transpose(0,1)).transpose(0,1))
...
##        self.fc1 = nn.Sequential(nn.Linear(250, 128),
##                                    nn.ReLU(inplace=True), # hidden layer 
##                                    nn.Linear(128, 1)) # output layer # used for simsiam loss
        if img_size == 96:
            self.fc2 = nn.Linear(27, 1)
##            self.fc2 = nn.Linear(216, 1)
        elif img_size == 128:
...
class Unidir_LocalCrossAttention(nn.Module):
    def __init__(self, embed_dim, drop_rate=0):
        super(Unidir_LocalCrossAttention, self).__init__()
        self.embed_dim = embed_dim
        self.key1 = nn.Linear(embed_dim, embed_dim)
        self.value1 = nn.Linear(embed_dim, embed_dim)
        self.dropout1 = nn.Dropout(drop_rate)
...
class Unidir_LocalPatchCrossAttention(nn.Module):
    def __init__(self, embed_dim, drop_rate=0):
        super(Unidir_LocalPatchCrossAttention, self).__init__()
        self.embed_dim = embed_dim
        self.key1 = nn.Linear(embed_dim, embed_dim)
        self.dropout1 = nn.Dropout(drop_rate)
        self.query2 = nn.Linear(embed_dim, embed_dim)
    ...
    def forward(
        ...
        for t in range(attention_probs1.size(1)):
            context_layer1.append(self.avgpool(context_layer1_whole[:, t, :].transpose(0,1)).transpose(0,1))
        context_layer1 = torch.cat(context_layer1, dim=0)
