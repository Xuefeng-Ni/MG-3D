...
from torchmetrics import F1Score, AUROC, BLEUScore
...
def epoch_wrapup(pl_module, test=False):
    ...
    for loss_name, v in pl_module.hparams.config["loss_names"].items():
        ...
        elif loss_name == "local_itc":
            value = -getattr(pl_module, f"{phase}_{loss_name}_loss").compute()
            pl_module.log(f"{loss_name}/{phase}/accuracy_epoch", value)
            pl_module.log(f"{loss_name}/{phase}/loss_epoch", - value)
            getattr(pl_module, f"{phase}_{loss_name}_loss").reset()

        elif loss_name == "t3d_ir":
            value = -getattr(pl_module, f"{phase}_{loss_name}_loss").compute()
            pl_module.log(f"{loss_name}/{phase}/accuracy_epoch", value)
            pl_module.log(f"{loss_name}/{phase}/loss_epoch", - value)
            getattr(pl_module, f"{phase}_{loss_name}_loss").reset()

        elif loss_name == "t3d_itc":
            value = -getattr(pl_module, f"{phase}_{loss_name}_loss").compute()
            pl_module.log(f"{loss_name}/{phase}/accuracy_epoch", value)
            pl_module.log(f"{loss_name}/{phase}/loss_epoch", - value)
            getattr(pl_module, f"{phase}_{loss_name}_loss").reset()
...
def set_schedule(pl_module):
    ...
    head_names = ["mlm_head", "mim_head", "mim_swin_unter_head", "itm_head", "itc_head", "itc_agg_head",
                  "ir_head", "vqa_head", "cls_head", "irtr_head", "mlc_head", "clm_head"]
    ...
    if optim_type == "adamw":
        ...
        optimizer = torch.optim.Adam(optimizer_grouped_parameters, lr=lr, weight_decay=1e-8)
    elif optim_type == "sgd":
