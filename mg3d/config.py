...
ex = Experiment("mg3d", save_git_info=False)   #ptunifier task_finetune_clm_ctrg_chest_vision_only
...
def config():
    exp_name = "ptunifier"
    seed = 0
    datasets = ["clm_chest_ctrg"]
    loss_names = _loss_names({"swin_unetr_mim": 1, "sen_mlm": 1, "local_itc": 0.1}) 
    batch_size = 4   #32  16   4096
    ...
    vqa_label_size = 3129
    mlc_label_size = 14
    max_text_len = 250  #64 #48 40
    tokenizer = "StanfordAIMI/RadBERT"  #"allenai/biomed_roberta_base" #"bert-base-uncased"
    vocab_size = 28996  #50265  #30522
    ...
    data_root = "/scratch/medimgfmod/csexuefeng/pretrain_arrows_comma_ct_rate/"

    log_dir = "/home/xuefeng/PTUnifier-main_single_gpu/result"
    per_gpu_batchsize = 4   #8   0
    num_gpus = 1 #4 8
    devices = [5] #[4,5,6,7]    #[4,5,6,7]
    num_nodes = 1
    load_path = ""
    num_workers = 8 #28  #8
    precision = 16
    ...
    pseudo_langauge_token_pool_size = 0 #1024
    num_pseudo_vision_tokens = 0    #49
    num_pseudo_langauge_tokens = 0 #32
...
def task_pretrain_ptunifier():
    exp_name = "task_pretrain_mg3d"
    datasets = ['clm_chest_ctrg']  
    loss_names = _loss_names({"swin_unetr_mim": 1, "sen_mlm": 1, "local_itc": 0.1})
    batch_size = 4  
    max_epoch = 60  #10
    max_steps = 300000  #100000
    warmup_steps = 10000    #0.1 10000
    whole_word_masking = True
...
@ex.named_config
def task_finetune_clm_ctrg_chest_vision_only():
    exp_name = "task_finetune_clm_ctrg_chest_vision_only"
    datasets = ["clm_chest_ctrg"]
    loss_names = _loss_names({"clm": 1})
    batch_size = 2
    max_epoch = 15
    ...
    lr_multiplier_head = 50
    lr_multiplier_multi_modal = 5
    tokenizer = "allenai/biomed_roberta_base"   #"bert-base-uncased"
    max_text_len = 256
    input_text_embed_size = 768
    vit = 'ViT-B/32'
    train_transform_keys = ["clip_resizedcrop"]
