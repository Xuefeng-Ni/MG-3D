...
from prepro.make_arrow_nii_jpg import make_arrow, make_arrow_mimic_cxr
...
def prepro_ctrg(min_length=3):
    ...
    filename = f'/home/csexuefeng/label/chest/chest_ctrg_jpg.json'
    ...
    for i in range(len(ann)):
        ...
        texts = ["CT Findings: " + report + "CT Impression: " + result]
        texts = [re.sub(r"\s+", " ", text.strip()) for text in texts]
        texts = [text for text in texts if len(text.split()) >= min_length]
    ...
    make_arrow(data, "chest_ctrg", "/scratch/medimgfmod/CT/pretrain_arrows/")
